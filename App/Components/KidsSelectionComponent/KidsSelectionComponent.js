...
this.getUserData();
 ...
 selectKidsCount(){
    ...
    if(this.state.countKids==0){
        alert("Please Select the Kids")}

    else if(this.state.enrolledKidsCount==0){
           alert("Please Enroll Kid")
        }
       
        else {Actions.whattobuy({type:'reset'})}
      ...
      return (
        ...
        <Modal
          ...
          <View style={{marginTop: 200,justifyContent:"center",alignItems:"center"}}>
           ...
           buttonColor={'black'}
           buttonOuterColor={"black"}
           onPress={(value) => {this.selectMoreEnrolled(value)}}
             />
            </View>
               <View  style={{height:1,backgroundColor:"grey"}} /> 
                <View style={{flexDirection:"row",paddingLeft:90,position:"absolute",bottom:20,justifyContent:"flex-end" }} >
